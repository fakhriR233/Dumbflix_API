...
type EpisodeRepository interface {
	...
	UpdateEpisode(episode models.Episode, ID int) (models.Episode, error)
...
func (r *repository) UpdateEpisode(episode models.Episode, ID int) (models.Episode, error) {
	err := r.db.Preload("Film").Raw("UPDATE episodes SET title=?, thumbnail_film=?, link_film=?,film_id=? WHERE id=?", episode.Title,episode.ThumbnailFilm, episode.LinkFilm, episode.FilmID, ID).Scan(&episode).Error
